					...
					<section class="panel">
						...
						<div class="panel-body">
							<div class="table-responsive">
  							<form method="post" novalidate="novalidate" action="{{ url('/d3ti-admin/post_categories/process') }}" enctype="multipart/form-data">
  								@csrf
								...
								<table class="table table-bordered table-striped mb-none" id="datatable-default">
									...
									<tbody>
										@foreach($data_post_categories as $data)
										    @php
										        $formId = "delete-form-" . $data->post_categories_id;
										    @endphp
										    <tr>
										        <td>{{ $data->post_categories_id }}</td>
										        <td>
										            {{ $data->post_categories_name }}
										            <br/>
										            <a class="modal-with-form" href="#editCategory{{ $data->post_categories_id }}">Edit</a> |
										            <a href="#" onclick="event.preventDefault(); if(confirm('Delete Category?')){document.querySelector('#{{ $formId }}').submit();}">Delete</a>
										            <form id="{{ $formId }}" method="post" action="{{ url ('d3ti-admin/delete_categories') }}/{{ $data->post_categories_id }}">
										                @csrf
										                @method('DELETE')
										            </form>
										        </td>
										        <td>{{ $data->post_categories_description }}</td>
										        <td>{{ $data->post_categories_url }}</td>
										    </tr>

										    <div id="editCategory{{ $data->post_categories_id }}" class="modal-block modal-block-primary mfp-hide">
										        <section class="panel">
										            <header class="panel-heading">
										                <h2 class="panel-title">Edit Category</h2>
										            </header>
										            <div class="panel-body">
										                <form id="editCategoryForm{{ $data->post_categories_id }}" method="post" action="{{ url('d3ti-admin/edit_categories/process/'.$data->post_categories_id) }}" class="form-horizontal mb-lg" novalidate="novalidate">
										                    @csrf
               												@method('PUT')
										                    <div class="form-group mt-lg">
										                        <label class="col-sm-3 control-label">Name</label>
										                        <div class="col-sm-9">
										                            <input type="text" name="name" class="form-control" value="{{ $data->post_categories_name }}" required/>
										                        </div>
										                    </div>
										                    <div class="form-group">
										                        <label class="col-sm-3 control-label">Description</label>
										                        <div class="col-sm-9">
										                            <input type="text" name="description" class="form-control" value="{{ $data->post_categories_description }}" required/>
										                        </div>
										                    </div>
										                    <div class="form-group">
										                        <label class="col-sm-3 control-label">URL</label>
										                        <div class="col-sm-9">
										                            <input type="text" name="url" id="editUrl{{ $data->post_categories_id }}" class="form-control" value="{{ $data->post_categories_url }}" required/>
										                        </div>
										                    </div>
										                </form>
										            </div>
										            <footer class="panel-footer">
										                <div class="row">
										                    <div class="col-md-12 text-right">
										                        <input type="submit" form="editCategoryForm{{ $data->post_categories_id }}" name="submit" class="btn btn-primary" value="Submit">
