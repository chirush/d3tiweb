...
@php
	$selectedProductCategories = getSelectedProductCategories($data['product_id']);
	$nonSelectedProductCategories = getNonSelectedProductCategories($data['product_id']);
	$selectedProductTags = getSelectedProductTags($data['product_id']);
	$nonSelectedProductTags = getNonSelectedProductTags($data['product_id']);
	$status = $data['product_status']
@endphp
<head>
	@if($status == "Pending")
	<title>D3 Teknik Informatika - Review Product</title>
	@else
	<title>D3 Teknik Informatika - Edit Product</title>
	@endif
...
<section role="main" class="content-body">
	<header class="page-header">
		...
		<div class="right-wrapper pull-right">
			...
			<form method="post" novalidate="novalidate" action="{{ url ('/d3ti-admin/edit_product/process/'.$data['product_id']) }}" enctype="multipart/form-data" class="form-horizontal form-bordered">
				@csrf
               	@method('PUT')
               	@if($status == "Published")
				...
				@endif
				@if($status == "Pending")
				<a href="{{ url('/d3ti-admin/review_product/preview/'.$data['product_link']) }}" class="btn btn-default" style="margin-left: 10px; color: black;">Preview</a>
				@else
					...
					<div class="form-group">
						...
						<div class="form-group">
							...
							<div class="col-md-9">
								...
								<input type="text" name="title" class="col-md-12" value="{{ $data['product_title'] }}">
							</div>
						</div>
						...
						<div class="form-group">
						    ...
						    <div class="col-md-9">
						        <img class="img-fluid w-100" id="featured-image" src="{{ url('storage/featured_images/'.$data['product_featured_image']) }}" height="100px" style="border-radius: 5%; margin-bottom: 10px;">
						        <input type="file" name="featured" accept="image/png, image/jpg, image/jpeg" onchange="previewImage(event)">
						    </div>
						</div>

						<div class="form-group">
							<textarea name="content" id="content">{{ $data['product_content'] }}</textarea>
						</div>
						...
						<div class="form-group" style="margin-top: 50px;">
							...
							<div class="col-md-9">
				                ...
				                @enderror
								<input type="text" id="owner" name="owner" class="col-md-12" value="{{ $data['product_owner'] }}">
							</div>
						</div>
						...
						<div class="form-group">
						    ...
						    <div class="col-md-9">
						        <input type="date" name="release_date" class="col-md-12 datepicker" value="{{ $data['product_release_date'] }}">
						    </div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-md-9">
								<select multiple data-plugin-selectTwo class="form-control populate" name="category[]">
									@foreach ($selectedProductCategories as $category)
									    <option value="{{ $category->product_categories_id }}" selected>{{ $category->product_categories_name }}</option>
									@endforeach

									@foreach($nonSelectedProductCategories as $category)
				                    	<option value="{{ $category->product_categories_id }}">{{ $category->product_categories_name }}</option>
				                  	@endforeach
								</select>
						...
						<div class="form-group">
							...
							<div class="col-md-9">
								<textarea name="excerpt" class="col-md-12" data-plugin-maxlength maxlength="120" style="height: 150px;">{{ $data['product_excerpt'] }}</textarea>
							</div>
						</div>
						...
						<div class="form-group">
						    ...
						    <div class="col-md-9">
						        <input type="datetime-local" name="date" class="col-md-12" value="{{ $data['product_date'] }}">
						    </div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-md-9">
								http://localhost/<input type="text" name="link" class="col-md-12" value="{{ $data['product_link'] }}" readonly>
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-md-9">
								<select multiple data-plugin-selectTwo class="form-control populate" name="tags[]">
									@foreach($selectedProductTags as $tag)
				                    	<option value="{{ $tag->tags_id }}" selected>{{ $tag->tags_name }}</option>
				                  	@endforeach

									@foreach($nonSelectedProductTags as $tag)
						...
						<div class="form-group">
							...
							<div class="col-md-9">
								<textarea name="meta" class="col-md-12" style="height: 150px;">{{ $data['product_meta'] }}</textarea>
							</div>
						</div>
					</form>
					@endforeach
